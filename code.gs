...
function doPost(e) {
  ...
  try {
    ...
    if (data.scrip && data.reason && !data.ticker) {
      ...
      if (!ind1Sheet) throw new Error(`Sheet not found: ${SHEET_INDICATOR_1}`);
      ind1Sheet.appendRow([ date, time, data.scrip, data.reason, 'Awaiting', '', '' ]);
      checkForSync(ss, data.scrip, date);
    }
    // --- Data from Indicator 2 ---
      ...
      if (!ind2Sheet) throw new Error(`Sheet not found: ${SHEET_INDICATOR_2}`);
      ind2Sheet.appendRow([ date, time, data.ticker, data.reason, data.capital_deployed_cr || '' ]);
